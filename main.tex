...
Deterministic scheduling and only having a single execution thread guaranteed that all requests were executed deterministically, and therefore that all replicas converge in the system without the need for checkpointing or potentially rolling back.

In figure \ref{seqpipe} you can see the execution of $12$ client requests in a single-threaded pipelined system. Notice that the grey squares indicate local computation, or that the replica machine is actually doing work. Only one thread is ever performing local computation at a time (guaranteeing determinism), but because of pipelining, the total number of time slices is cut almost in half over the naive approach.
