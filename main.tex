...
When implementing Adam with the multi-threaded pipelined approach, we decided to first combine the two existing approaches into one system (described in \ref{existing}) before implementing the parts of the functionality that were not yet present. 
...
By taking the structure of the existing SPM written by Kapritsos, we built what we call the Parallel Pipeline Manager (PPM) to schedule groups of threads in the context of the pipeline. 
...
The second part of our system that allowed groups of threads to run in a speculative manner was the Parallel Group Manager (PGM). This was the part of the system that controlled the execution threads within a single group.
