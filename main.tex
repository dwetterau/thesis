...
When implementing Adam with the multi-threaded pipelined approach, we decided to first combine the two existing approaches into one system (described in \ref{existing}) in a very structured way before implementing the parts of the functionality that were not yet present. 
...
After these modifications, we finished implementing parts of Adam that had not been implemented before such as the verification stage and made appropriate modifications to other existing parts of the code base to support both multi-threading and pipelining simultaneously.
